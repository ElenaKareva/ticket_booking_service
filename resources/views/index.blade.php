...
<body>
  <header class="page-header">
    ...
    <div class="block_login"> <a href="{{ route('admin') }}" class="link_login font-semibold text-gray-600 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white focus:outline focus:outline-2 focus:rounded-sm focus:outline-red-500">Администратор</a></div>
  </header>
  ...
  @foreach($sessions->groupby('date') as $date => $date_value)
    <main class="hidden" style="display:none">
      @foreach($date_value->groupby('title_film') as $movie => $movies)
      <section class="movie">
